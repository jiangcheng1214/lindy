...
from Utils import log_info, get_current_pst_time, log_warning, ConsecutiveTimeoutsException, ConsecutiveBlocksException
        ...
        while 1:
            ...
            if scrape_flag == "BLOCKED":
                ...
                if scrape_flag == "SUCCESS":
                    ...
                    if time_used_in_seconds > 120:
                        sequential_timeouts += 1
                        if sequential_timeouts == 3:
                            if timeout_resets == 2:
                                raise ConsecutiveTimeoutsException("Scraper failed for 3 times in a row")
                            else:
                                timeout_resets += 1
                                sequential_timeouts = 0
                                self.scraper.terminate()
                                log_warning("Starting new scraper instance after sequential timeout..")
                                self.scraper = Scraper(proxy=self.get_proxy(get_next=True), headless=not self.debug)
                    else:
                        sequential_timeouts = 0
                        timeout_resets = 0
            ...
            if sequential_blocks == 3:
                raise ConsecutiveBlocksException("Scraper failed for 3 times in a row")
            index += 1
